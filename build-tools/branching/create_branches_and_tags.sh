...
usage () {
    ...
    echo "    [ --manifest [ --manifest-file=<file.xml> ] [ --hard-lock-down  | --soft-lock-down ] [ --default-revision ]]"
    echo ""
    echo "    - If a manifest is requested, the current mannifest is modified"
    echo "      to specify the new branch for all select remotes and projects."
    echo "    - If hard-lock-down is requested, all unselected projects get the current"
    echo "      HEAD's sha set as the revision."
    echo "    - If soft-lock-down is requested, all unselected projects with an revision"
    echo "      that is unset, or 'master', get the current HEAD's sha set as the revision."
